/**
 * Standard normal CDF and inverse for the truncated-Gaussian sampler,
 * standing in for scipy.special.ndtr/ndtri in the Python package.
 *
 * Φ comes from W. J. Cody's rational erfc (SPECFUN calerf), full double
 * precision with correct relative accuracy deep in the lower tail — which is
 * what the mirrored truncated sampler relies on. Φ⁻¹ is Acklam's
 * approximation (~1.15e-9 relative) polished by one Halley step against this
 * Φ, which reaches ~1e-13 everywhere the step is taken: down to x ≈ -37.5
 * (below, exp(x²/2) overflows) and up to p = 1 - 1e-9 (beyond, the spacing
 * of doubles near 1 limits any method to about what Acklam already gives).
 */
...
export function ndtri(p: number): number {
  ...
  // One Halley step against the accurate Φ (see the header for the guards).
  if (p <= 1 - 1e-9 && x > -37.5) {
    const e = ndtr(x) - p
